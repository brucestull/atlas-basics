...
# Create/use a database named "atlasBasicsDatabase"
db = client.atlasBasicsDatabase
print(f"db: {db}")

# Create/use a collection named "atlasBasicsCollection" which is part of the "atlasBasicsDatabase" database.
atlas_basics_collection = db["atlasBasicsCollection"]
print(f"atlas_basics_collection: {atlas_basics_collection}")

current_environment = os.getenv("CURRENT_ENVIRONMENT")
print(f"current_environment: {current_environment}")

current_collection = atlas_basics_collection.find()
...
atlas_basics_documents = [
    {
        "current_environment": current_environment,
        "date_time_string": date_time_string,
    },
    {
        "application": "atlas-basics",
    }
]
print(f"atlas_basics_documents: {atlas_basics_documents}")

# Try to drop the collection in case it already exists.
try:
    print("Dropping collection...")
    atlas_basics_collection.drop()
    print("Collection dropped")
# Throw an exception if `atlas_basics_collection` collection does not exist.
...
try:
    ...
    result = atlas_basics_collection.insert_many(atlas_basics_documents)
